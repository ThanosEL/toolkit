...
// type will have access to all the methods with the receiver of *Tools.
type Tools struct {
	MaxFileSize        int
...
// UploadedFile is the struct used to hold information about a file that is being uploaded.
...
// ReadJSON tries to read the body of the request and convert it from JSON to a go data variable
func (t *Tools) ReadJSON(w http.ResponseWriter, r *http.Request, data interface{}) error {
	...
	if err != nil {
		...
		case errors.As(err, &unmarshalTypeError):
			...
			return fmt.Errorf("body contains incorrect JSON type for field at position %d", unmarshalTypeError.Offset)
...
// ErrorJSON takes an error and an optional status code, and sends back a JSON response with the error message
...
// The final parameter is an optional http.Client. If one is not provided, the default http.Client will be used.
func (t *Tools) PushJSONToRemote(uri string, data interface{}, client ...*http.Client) (*http.Response, int /*status code*/, error) {
	// create json
